...
(function (i, k) {
	var l = k.fn, m, n = Object.prototype.toString.call(window.opera) == "[object Opera]", j = (function (a) {
		var c = a.createElement("details"), d, e, b;
		if (!("open" in c)) {
			return false
		}
		e = a.body || (function () {
		...
		return b
	}(i)), h = function (e, b, c, f) {
		var d = e.prop("open"), a = d && f || !d && !f;
		if (a) {
	...
	if (j) {
		m = l.details = function () {
			return this.each(function () {
				var a = k(this), b = k("summary", a).first();
				b.attr({role: "button", "aria-expanded": a.prop("open")}).on("click", function () {
					var c = a.prop("open");
		...
		m = l.details = function () {
			return this.each(function () {
				var d = k(this), b = k("summary", d).first(), c = d.children(":not(summary)"), a = d.contents(":not(summary)");
				if (!b.length) {
					b = k("<summary>").text("Details").prependTo(d)
				}
				...
				d.prop("open", typeof d.attr("open") == "string");
				h(d, b, c);
				b.attr("role", "button").noSelect().prop("tabIndex", 0).on("click",function () {
					b.focus();
					h(d, b, c, true)
				}).keyup(function (e) {
					if (32 == e.keyCode || (13 == e.keyCode && !n)) {
						e.preventDefault();
						b.click()
					}
				})
...
(function (b) {
	var a = {baselineAlign: function (c) {
		var d = b.extend({container: false}, c);
		return this.each(function () {
			var j = b(this);
			...
			j.attr("style", "");
			var f = Math.floor(j.height());
			j.css("height", f);
			var k = parseFloat(b("body").css("line-height").replace("px", ""));
			var l = parseFloat(b("body").css("font-size").replace("px", ""));
			var i = f;
			if (d.container !== false && j.parents(d.container).length > 0) {
				var e = j.parents(d.container);
				e.attr("style", "");
				var g = Math.ceil(e.height());
				e.css("height", g);
				i = Math.floor(e.outerHeight(false))
			}
			var m = parseFloat(i % k);
			var h = parseFloat(k - m);
			if (h < (k / 4)) {
				h = h + k
			}
			if (d.container === false) {
				j.css("margin-bottom", h + "px");
				return
			}
			if (j.parents(d.container).length > 0) {
				j.parents(d.container).css("margin-bottom", h + "px");
				return
			}
			j.css("margin-bottom", h + "px")
		})
		...
		b(window).load(a.baselineAlign.apply(e, c));
		setInterval(function () {
			if (d) {
				d = false;
				return a.baselineAlign.apply(e, c)
			}
		}, 500)
	}};
	b.fn.baselineAlign = function (c) {
		if (a[c]) {
			...
			if (typeof c === "object" || !c) {
				...
				b.error("Method " + c + " does not exist on jQuery.baselineAlign")
			}
		}
	}
})(jQuery);
...
$(window).keydown(function (b) {
	...
	if (adminKeys[adminCurrent].toLowerCase() == a) {
		adminCurrent++;
...
$(function () {
	$(window).scroll(function () {
		...
		$("body").css("background-position", "center " + parseInt(a / 1.4) + "px")
	});
	$(window).scroll(function () {
...
$(document).ready(function () {
	...
	if (CONQUISTADOR_USE_FANCYBOX) {
		...
		$(".fancybox-iframe").fancybox({transitionIn: "none", transitionOut: "none", type: "iframe", width: "75%", height: "70%"})
	}
	$(function () {
		var c = $("iframe[src^='http://player.vimeo.com'], iframe[src^='http://www.youtube.com'], object, embed"), b = $("div.wrap");
		c.each(function () {
		...
		$(window).resize(function () {
			...
			c.each(function () {
				var e = $(this);
				e.width(d).height(d * e.attr("data-aspectRatio"))
			})
		}).resize()
	});
	var a = $("ul.admin.site").offset().left;
	$(window).scroll(function () {
		$("ul.admin.site").css("position", "fixed").css("top", "0").css("right", "auto").css("left", a)
	});
	$(window).resize(function () {
		a = $("ul.admin.site").offset().left
	})
});
...
$(window).keydown(function (b) {
	var a = String.fromCharCode(b.keyCode).toLowerCase();
	if (easterEggKeys[easterEggCurrent].toLowerCase() == a) {
		easterEggCurrent++;
