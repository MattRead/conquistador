	...
	<div id="primary">
		<div class="navigation">
			<?php if ($previous = $post->descend()): ?>
				<div class="left"> &laquo; <a href="<?php echo $previous->permalink ?>"
				                              title="<?php echo $previous->slug ?>"><?php echo $previous->title ?></a>
				</div>
			<?php endif; ?>
			<?php if ($next = $post->ascend()): ?>
				<div class="right"><a href="<?php echo $next->permalink ?>"
				                      title="<?php echo $next->slug ?>"><?php echo $next->title ?></a> &raquo;</div>
			<?php endif; ?>

			<div class="clear"></div>
		</div>

		<?php if ($user->can('super_user', 'read')): ?><p>Please note this is just a default template, which should be
			replaced according to the installation instructions.</p><?php endif; ?>

		<div id="post-<?php echo $post->id; ?>" class="<?php echo $post->statusname; ?>">

			<div class="entry-head">
				<h3 class="entry-title">Event: <a href="<?php echo $post->permalink; ?>"
				                                  title="<?php echo $post->title; ?>"><?php echo $post->title_out; ?></a>
				</h3>
				<small class="entry-meta">
					<span class="chronodata"><abbr
							class="published"><?php $post->pubdate->out(); ?></abbr></span> <?php _e('by %s', array($post->author->displayname)); ?>
					<span class="commentslink"><a href="<?php echo $post->permalink; ?>#comments"
					...
					<?php if ($loggedin) { ?>
						<span class="entry-edit"><a href="<?php echo $post->editlink; ?>"
						                            title="<?php _e('Edit event'); ?>"><?php _e('Edit'); ?></a></span>
					<?php } ?>
			...
			<iframe width="425" height="350" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"
			        src="https://maps.google.com/maps?q=<?php echo $post->info->location; ?>&amp;source=embed&amp;output=svembed"></iframe>
			<br/>
			<small><a href="https://maps.google.com/maps?q=<?php echo $post->info->location; ?>&amp;source=embed"
