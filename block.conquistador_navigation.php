	...
	<nav class="posts">
		<p class="meta">
			<?php if ( $content->comment_count ): ?>
				<a href="#comments" title="Read Comments"><i class="icon-"></i> <?php echo $content->comment_count; ?> Comments</a> &nbsp;|&nbsp;
			<?php endif; ?>
			...
			<?php if (isset($content->previous) && $content->previous) : ?>
				<a href="<?php echo $content->previous->permalink; ?>"
				   title="<?php echo htmlentities($content->previous->title); ?>">Previous Post <i class="icon-arrow-right2"></i></a>
			<?php endif; ?>
		</p>
	</nav>
<?php endif; ?>
